...
# %%time
# response = model.generate_content("What is sundar pichai?")
...
# Commented out IPython magic to ensure Python compatibility.
# %%time
# response = model.generate_content("Who is sundar pichai?", stream=True)
...
!curl -o image.jpg https://c.ndtvimg.com/2023-06/tnsc6ik8_personalities-playing-holi_625x300_18_June_23.jpg
...
response = model.generate_content(["Write a short, engaging blog post based on this picture. In the first image, Russian President Vladimir Putin is seen covered in colorful powder. In the second image, North Korean leader Kim Jong-un is also seen covered in colorful powder. In the third image, Tesla CEO Elon Musk is seen covered in colorful powder. The images appear to have been taken during the Hindu holiday of Holi, which is a festival of colors.", img], stream=True)
response.resolve()
...
response = chat.send_message("In one sentence, explain 2024 generative ai future.")
to_markdown(response.text)
...
response = chat.send_message("Okay, is ai thread of humanity?", stream=True)
...
model.count_tokens("explain 2024 generative ai future.")
...
result = genai.embed_content(
    ...
    content="explain 2024 generative ai future",
...
result = genai.embed_content(
    model="models/embedding-001",
    content=[
      'explain 2024 generative ai future',
      'is ai threads of humanity?',
...
if parts[0].finish_reason.name == "MAX_TOKENS":
    parts[0].text += '...'

to_markdown(parts[0].text)
